...
var produtos = mysqlTable("produtos", {
  id: int("id").autoincrement().primaryKey(),
...
var galeria = mysqlTable("galeria", {
  id: int("id").autoincrement().primaryKey(),
...
var depoimentos = mysqlTable("depoimentos", {
  id: int("id").autoincrement().primaryKey(),
  nome: varchar("nome", { length: 255 }).notNull(),
...
var contatos = mysqlTable("contatos", {
  id: int("id").autoincrement().primaryKey(),
  nome: varchar("nome", { length: 255 }).notNull(),
...
async function getProdutos() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(produtos).orderBy(produtos.createdAt);
}
async function getGaleria() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(galeria).orderBy(galeria.createdAt);
}
async function getDepoimentos() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(depoimentos).orderBy(depoimentos.createdAt);
}
async function getContatos() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(contatos).orderBy(contatos.createdAt);
}
